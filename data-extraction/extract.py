...
def finalize_data(stock_csv_file, start, end, repartition):
    if not repartition:
        ...
        df = df.reset_index()
        df.to_csv(f'output/{stock_csv_file}', index=False, columns=['Date', 'Symbol', 'Open', 'High', 'Low', 'Close', 'Volume'])
        ...
        df_spark.write.parquet('output/stocks/data_raw/', mode='overwrite')
